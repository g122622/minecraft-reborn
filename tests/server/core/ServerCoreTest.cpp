...
TEST_F(ServerCoreTest, ForEachPlayer) {
    ...
    m_core->forEachPlayer([&ids](mr::server::ServerPlayerData& player) {
        ids.push_back(player.playerId);
    });

    EXPECT_EQ(ids.size(), 2u);
    EXPECT_TRUE(std::find(ids.begin(), ids.end(), mr::PlayerId(1)) != ids.end());
    EXPECT_TRUE(std::find(ids.begin(), ids.end(), mr::PlayerId(2)) != ids.end());
}
...
TEST_F(ServerCoreTest, TeleportPlayer) {
    ...
    mr::u32 teleportId = m_core->teleportPlayer(1, 100.0, 64.0, 200.0, 90.0f, 45.0f);
...
TEST_F(ServerCoreTest, ConfirmTeleport) {
    ...
    mr::u32 teleportId = m_core->teleportPlayer(1, 100.0, 64.0, 200.0);
...
TEST_F(ServerCoreTest, Tick) {
    ...
    mr::u64 initialTick = m_core->currentTick();
    m_core->tick();
