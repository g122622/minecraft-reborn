...
TEST_F(TerrainGeneratorTest, EmptyGenerator) {
...
TEST_F(TerrainGeneratorTest, FlatGenerator) {
    ...
    const BlockState* blockAtMin = chunk.getBlock(8, MIN_BUILD_HEIGHT, 8);
    ASSERT_NE(blockAtMin, nullptr);
    EXPECT_EQ(blockAtMin->blockId(), VanillaBlocks::BEDROCK->blockId());
...
TEST_F(TerrainGeneratorTest, StandardGenerator) {
...
TEST_F(TerrainGeneratorTest, BiomeGeneration) {
...
TEST_F(TerrainGeneratorTest, ChunkConsistency) {
