...
class ModAPI {
public:
    Result<void> spawnEntity(ModContext& ctx, EntityType type) {
        // 检查权限
        if (!ctx.hasPermission("entity.spawn")) {
            return Error::permissionDenied("entity.spawn");
        }
        
        // 检查资源限制
        if (ctx.getEntityCount() >= ctx.getMaxEntities()) {
            return Error::resourceExceeded("entity limit");
        }
...
class Sandbox {
public:
    Result<void> execute(const std::string& script) {
        auto startTime = std::chrono::steady_clock::now();
        
        auto result = m_context->evaluate(script);
        
        auto endTime = std::chrono::steady_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(
            endTime - startTime);
        
        if (duration > m_config.maxExecutionTime) {
            return Error::timeout("Script execution timeout");
        }
...
```cpp
// ✅ 推荐：验证网络数据包
Result<void> handlePacket(const Packet& packet) {
    // 验证包大小
    if (packet.size() > MAX_PACKET_SIZE) {
        return Error::invalidPacket("Packet too large");
    }
    
    // 验证包类型
    if (!isValidPacketType(packet.type())) {
        return Error::invalidPacket("Unknown packet type");
    }
    
    // 验证发送者状态
    if (!m_session.isAuthenticated()) {
        return Error::permissionDenied("Not authenticated");
    }
    
    // 速率限制
    if (!m_rateLimiter.allow()) {
        return Error::rateLimited("Too many requests");
    }
...
public:
    void save() {
        auto encrypted = encrypt(m_data, m_encryptionKey);
        writeFile("player.dat", encrypted);
    }
...
// TRACE: 最详细的调试信息（默认关闭）
SPDLOG_TRACE("Function entry, x = {}", x);

// DEBUG: 调试信息（开发环境启用）
SPDLOG_DEBUG("Loaded {} chunks", chunkCount);

// INFO: 一般信息（默认启用）
SPDLOG_INFO("Server started on port {}", port);

// WARN: 警告（不影响运行但需要注意）
SPDLOG_WARN("Chunk load time exceeded threshold: {}ms", loadTime);

// ERROR: 错误（操作失败但可恢复）
SPDLOG_ERROR("Failed to load texture: {}", path);

// CRITICAL: 严重错误（可能导致崩溃）
SPDLOG_CRITICAL("Out of memory, shutting down");
```
...
```cpp
// ✅ 推荐：结构化日志
SPDLOG_INFO("[World] Chunk loaded: x={}, z={}, time={}ms", 
            chunkX, chunkZ, loadTime);

// ✅ 推荐：包含上下文信息
logger->set_pattern("[%Y-%m-%d %H:%M:%S.%e] [%l] [%t] [%s:%#] %v");
// 输出：[2025-01-15 10:30:45.123] [info] [thread-1] [ChunkManager.cpp:42] Chunk loaded

// ❌ 禁止：模糊的日志
SPDLOG_INFO("Error occurred");  // ❌ 什么错误？哪里？

// ✅ 推荐：详细的错误日志
SPDLOG_ERROR("Failed to load chunk at ({}, {}): {}", 
             x, z, result.error().message());
```
...
```cpp
// ✅ 推荐：条件日志
if (SPDLOG_LEVEL_ENABLED(spdlog::level::debug)) {
    SPDLOG_DEBUG("Expensive debug info: {}", computeExpensiveString());
}

// ✅ 推荐：异步日志（生产环境）
auto logger = spdlog::basic_logger_mt("async_logger", "logs/minecraft.log");
spdlog::init_thread_pool(8192, 1);  // 8KB队列，1个线程
```
...
| 类型 | 说明 | 示例 |
|------|------|------|
| `feat` | 新功能 | `feat(renderer): add shadow mapping` |
| `fix` | 修复bug | `fix(network): packet desync issue` |
| `docs` | 文档更新 | `docs: update API documentation` |
| `style` | 代码格式 | `style: format code with clang-format` |
| `refactor` | 重构 | `refactor(world): extract chunk manager` |
| `perf` | 性能优化 | `perf(renderer): reduce draw calls` |
| `test` | 测试相关 | `test: add chunk serialization tests` |
| `chore` | 构建/工具 | `chore: update vcpkg dependencies` |

### 13.3 提交消息示例

```
feat(modding): add JavaScript event system

- Add EventBus for mod-to-core communication
- Implement player.join and player.leave events
- Add mod permission system for event subscription

Closes #123
Breaks change: Event API signature changed
...
```cpp
// ✅ 推荐：具体的反馈
// 建议：使用std::unique_ptr代替裸指针，避免内存泄漏
// 位置：ChunkManager.cpp:45
...
// ✅ 推荐：提供解决方案
// 建议：预分配vector容量
// 当前：vec.push_back(i) 在循环中
// 建议：vec.reserve(1000) 在循环前
